...
public class UserServiceImpl implements UserService {
	...
	public List<UserAcctResource> getUserAccounts(Long userId) {
		 ...
		 */
		/*
		 * mapper.typeMap(AccountEntity.class,
		 * UserAcctResource.class).addMappings(mapper -> { mapper.map(src ->
		 * src.getAcctNumber(), UserAcctResource::setAcctNumber); mapper.map(src ->
		 * src.getAcctName(), UserAcctResource::setAcctName); mapper.map(src ->
		 * src.getCurrency(), UserAcctResource::setCurrency); mapper.map(src ->
		 * src.getAcctType(), UserAcctResource::setAcctType); });
		 */
